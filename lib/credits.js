...
// Atomically claim a one-time key (e.g. a sale id). Returns:
//   true  → newly claimed (first time — caller should proceed)
//   false → already existed (duplicate — caller should skip)
//   null  → KV unavailable (caller decides; default to best-effort proceed)
export async function claimOnce(key, ttlSeconds = 60 * 60 * 24 * 90) {
  const hasKV = await initKV();
  if (!hasKV) return null;
  try {
    // SET key val NX EX ttl → "OK" when set, null when key already exists.
    const res = await kv.set(key, Date.now(), { nx: true, ex: ttlSeconds });
    return res !== null;
  } catch {
    return null;
  }
