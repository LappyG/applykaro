export async function analyzeWithAI(resumeText, jdText) {
  const systemPrompt = `You are an expert ATS (Applicant Tracking System) resume analyzer. Analyze the resume against the job description and return ONLY a valid JSON object with no markdown, no backticks, no preamble. The JSON must have this exact structure:
...
- Recognize diverse education formats (B.Tech, M.Tech, BS, MS, MBA, etc.)
